...
def hook_sift_layer(model, teacher, hidden_size, learning_rate=1e-4, init_perturbation=1e-2, target_module = 'embeddings.LayerNorm'):
  """
  ...
  if isinstance(target_module, str):
    ...
    _teacher_modules = [k for n,k in teacher.named_modules() if  target_module in n]
  else:
  ...
  for i, m in enumerate(_modules):
    ...
    for x in list(_teacher_modules[i]._forward_pre_hooks.keys()):
      if _teacher_modules[i]._forward_pre_hooks[x].__name__ == 'adv_hook':
        del _teacher_modules[i]._forward_pre_hooks[x]

    m.register_forward_pre_hook(adv_hook)
    _teacher_modules[i].register_forward_pre_hook(adv_hook)
...
class AdversarialLearner:
  ...
  """
  def __init__(self, model, teacher, adv_modules=None):
    ...
    self.teacher_parameters = [p for p in model.parameters()] 
    self.model = model
    self.teacher = teacher

  def loss(self, target, teacher_target, logits_fn, loss_fn = 'symmetric-kl', *wargs, **kwargs):
    """
    ...
    pert_loss = (1 - 1/2 * (kl(pert_logits, M) + kl(teacher_pert_logits, M))).sum()
